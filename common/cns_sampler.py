...
from stable_baselines3 import RDPPO_onepolicy, RDPPO_twopolicy, RPPOLagrangian
from stable_baselines3.iteration import RobustDualPolicyIteration
...
class ConstrainedRLSampler:
    ...
    def sample_from_agent(self, policy_agent, new_env, test_op_strength=0):
        self.env = new_env
        ...
        if self.apply_planning:
            ...
            if self.sample_multi_env:
                return self.multi_threads_sample_with_policy(test_op_strength)
            else:
                return self.single_thread_sample_with_policy(test_op_strength)

    def single_thread_sample_with_policy(self, test_op_strength):
        ...
        for i in range(self.rollouts):
            ...
            acs_game = []
            rs_game = []
            if isinstance(self.policy_agent, RDPPO_onepolicy) or isinstance(self.policy_agent, RDPPO_twopolicy)\
                    or isinstance(self.policy_agent, RPPOLagrangian) or isinstance(self.policy_agent, RobustDualPolicyIteration):
                while not done:
                    action, state = self.policy_agent.mix_predict(obs, state=state,
                                                                  deterministic=False,
                                                                  test_op_strength=test_op_strength)
    ...
    def multi_threads_sample_with_policy(self, test_op_strength):
