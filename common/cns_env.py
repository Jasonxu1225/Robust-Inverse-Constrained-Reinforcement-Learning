...
def make_train_env(env_id, config_path, save_dir, group='PPO', base_seed=0, num_threads=1,
    ...
    if config_path is not None:
        ...
        if 'Noise' in env_id:
            env_configs = {'noise_mean': kwargs['noise_mean'], 'noise_std': kwargs['noise_std'],
                           'noise_seed': kwargs['noise_seed'], 'noise_type': kwargs['noise_type']}
        else:
            env_configs = {}
    ...
    if use_cost:  # add external cost wrapper for reading cost info
        if group == 'PPO-Lag' or group == 'PI-Lag' or group == 'DPI' or group == 'RDPI' or group == 'RDVI'\
                or group == 'VI-Lag' or group == '2VI-Lag' or group == '2SoftVI-Lag':
...
class InternalVecCostWrapper(VecEnvWrapper):
    ...
    def get_info(self):
        info = self.venv.get_info()
        return info

    def recover_with_info(self, info_dicts):
        obs = self.venv.recover_with_info(info_dicts)
        self.previous_obs = obs
        return obs
