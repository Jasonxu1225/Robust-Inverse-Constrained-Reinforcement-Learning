...
def traj_visualization_2d(config, observations, save_path, model_name='', title='', axis_size=16):
    traj_num = len(observations)
    import matplotlib as mpl
    mpl.rcParams['xtick.labelsize'] = axis_size
    mpl.rcParams['ytick.labelsize'] = axis_size
    plt.figure(figsize=(5, 5))
    # for i in range(traj_num)[0: traj_num: traj_num//5]:
    ...
    plt.grid(linestyle='--')
    plt.title('{0}'.format(title), fontsize=18)
...
def constraint_visualization_2d_matrix(matrix, feature_range, title='',  save_path=None, model_name='', axis_size=20):
