...
def evaluate_icrl_policy(
    ...
    for i in tqdm(range(n_eval_episodes)):
        ...
        episode_length = 0
        while not done:
            action, state = model.mix_predict(obs, state=state, deterministic=deterministic, test_op_strength=test_op_strength)
            ...
            for i in range(env.num_envs):
                if 'cost' in _info[i].keys():
                    ...
                    true_costs.append(_info[i]['lag_cost'])
                else:
                    costs = None
                    true_costs = None
                for record_info_name in record_info_names:
        ...
        if is_constraint[i] == True:
            constraint_episode += 1
    ...
    return mean_reward, std_reward, mean_nc_reward, std_nc_reward, record_infos, \
           costs, true_costs, constraint_episode*1.0/n_eval_episodes

def evaluate_icrl_policy_cost_episode(
        model: "base_class.BaseAlgorithm",
        env: Union[gym.Env, VecEnv],
        record_info_names: list,
        n_eval_episodes: int = 10,
    ...
    for i in tqdm(range(n_eval_episodes)):
        ...
        episode_reward = np.asarray([0.0] * env.num_envs)
        episode_nc_reward = np.asarray([0.0] * env.num_envs)
        episode_cost = np.asarray([0.0] * env.num_envs)
        is_constraint = [False for i in range(env.num_envs)]
        episode_length = 0
        while not done:
            action, state = model.mix_predict(obs, state=state, deterministic=deterministic, test_op_strength=test_op_strength)
            ...
            if 'admissible_actions' in _info[0].keys():
                model.admissible_actions = _info[0]['admissible_actions']
            for i in range(env.num_envs):
                if 'lag_cost' in _info[i].keys():
                    episode_cost[i] += _info[i]['lag_cost']
                    costs.append(_info[i]['lag_cost'])
                else:
                    episode_cost[i] += 0
                    costs = None
                for record_info_name in record_info_names:
    ...
    if return_episode_rewards:
        return episode_rewards, episode_lengths
    return mean_reward, std_reward, mean_nc_reward, std_nc_reward, record_infos, \
           mean_cost, std_cost, constraint_episode*1.0/n_eval_episodes, costs

def evaluate_icrl_policy_cost_episode_withop(
        model: "base_class.BaseAlgorithm",
        op_model: "base_class.BaseAlgorithm",
        env: Union[gym.Env, VecEnv],
        record_info_names: list,
        n_eval_episodes: int = 10,
        deterministic: bool = True,
    ...
    for i in tqdm(range(n_eval_episodes)):
        ...
        while not done:
            if np.random.random() < test_op_strength:
                action, state = op_model.policy.op_predict(obs, state=state, deterministic=deterministic)
            else:
    ...
    if return_episode_rewards:
        return episode_rewards, episode_lengths
    return mean_reward, std_reward, mean_nc_reward, std_nc_reward, record_infos, \
           mean_cost, std_cost, constraint_episode*1.0/n_eval_episodes
