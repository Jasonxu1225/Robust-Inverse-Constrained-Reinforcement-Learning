...
class DualPolicyIteration(ABC):
    ...
    def __init__(self,
        ...
        super(DualPolicyIteration, self).__init__()
    ...
    def _setup_model(self) -> None:
        ...
        self.q = np.zeros((self.height, self.width, self.n_actions))
        self.q_safety = np.zeros((self.height, self.width, self.n_actions))
    ...
    def learn(self,
              ...
              callback=None, ):
        policy_stable, policy_safety_stable = False, False
        iter = 0
        for iter in tqdm(range(total_timesteps)):
            self.num_timesteps += 1
            for i in range(self.max_iter):
                if policy_safety_stable:
                    print("\nSafety Policy Stable at Iteration {0}.".format(i), file=self.log_file)
                    break
                self.policy_evaluation_safety(cost_function)
                policy_safety_stable = self.policy_improvement_safety(cost_function)
            if policy_stable :
                print("\nTask Policy Stable at Iteration {0}.".format(iter), file=self.log_file)
                break
    ...
    def policy_evaluation_safety(self, cost_function):
        ...
        while delta >= self.stopping_threshold and iter <= self.max_iter-1:
            ...
            for x in range(self.height):
                for y in range(self.width):
                    # Run one iteration of the Bellman update rule for the value function
                    self.bellman_update_safety(old_v, x, y, cost_function)
                    # Compute difference
        ...
        for x in range(self.height):
            for y in range(self.width):
                ...
                if [x,y] in robust_invariant_set:
                    for action in range(self.n_actions):
                        if self.q_safety[x,y,action] < -self.cost_threshold:
                            q[x, y, action] = 0
    ...
    def policy_improvement_safety(self, cost_function):
        ...
        for x in range(self.height):
            for y in range(self.width):
                old_pi = self.pi_safety[x, y, :].copy()
                ...
                if not (old_pi == self.pi_safety[x, y, :]).all():
                    policy_stable = False
    ...
    def bellman_update(self, old_v, x, y, cost_function):
        ...
        for action in range(self.n_actions):
            ...
            for i in range(self.n_sample):
                ...
                total_oldv = total_oldv + old_v[s_primes[0][0], s_primes[0][1]]

            avg_reward = total_reward / (1.0*self.n_sample)
            avg_oldv = total_oldv / (1.0*self.n_sample)

            self.q[x, y, action] = (avg_reward + self.gamma * avg_oldv)
            total += self.pi[x, y, action] * self.q[x, y, action]
        self.v[x, y] = total

    def bellman_update_safety(self, old_v, x, y, cost_function):
        if [x, y] in self.terminal_states:
            return
        total = 0
        for action in range(self.n_actions):
            ...
            for i in range(self.n_sample):
                ...
                total_cost = total_cost + orig_costs[0]
                total_oldv = total_oldv + old_v[s_primes[0][0], s_primes[0][1]]

            avg_cost = total_cost / (1.0*self.n_sample)
            avg_oldv = total_oldv / (1.0*self.n_sample)

            self.q_safety[x, y, action] = (-avg_cost + self.gamma * avg_oldv)
            total += self.pi_safety[x, y, action] * self.q_safety[x, y, action]
        self.v_safety[x, y] = total
    ...
    def get_robust_invariant_set(self):
        ...
        for x in range(self.height):
            for y in range(self.width):
                if np.max(self.q_safety, axis=2)[x][y] >= -self.cost_threshold:
...
def load_pi(model_path, iter_msg, log_file):
    ...
    create_iteration_agent = lambda: DualPolicyIteration(
        env=None,
        max_iter=max_iter,
        n_actions=n_actions,
