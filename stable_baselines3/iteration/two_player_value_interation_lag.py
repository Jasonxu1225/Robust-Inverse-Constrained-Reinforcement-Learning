...
class TwoPlayerValueIterationLagrange(ABC):
    ...
    def __init__(self,
        ...
        super(TwoPlayerValueIterationLagrange, self).__init__()
    ...
    def get_two_policy(self, cost_function):
        q_policy = np.min(self.q2p, axis = 3)
        ...
        for x in range(self.height):
            for y in range(self.width):
                ...
                best_action_indices = np.where(q_policy[x, y, actions] ==
                                                np.max(q_policy[x, y, actions]), True, False)
    ...
    def optimal_bellman_update(self, old_v, x, y, cost_function):
        ...
        self.v_m[x, y] = np.max(np.min(self.q2p, axis=3), axis=2)[x][y]
...
def load_pi(model_path, iter_msg, log_file):
    ...
    create_iteration_agent = lambda: TwoPlayerValueIterationLagrange(
        env=None,
        max_iter=max_iter,
        n_actions=n_actions,
