...
class ValueIterationLagrange(ABC):
    ...
    def __init__(self,
        ...
        super(ValueIterationLagrange, self).__init__()
    ...
    def learn(self,
        ...
        self.find_optimal_policy(cost_function)
        ...
        # plt.matshow(self.v_m)
        # plt.show()
        # print(np.round(np.flipud(self.v_m), 4))
        # print('[2]---------rlud---------')
        # print(self.pi[2][0])
        # print(self.pi[2][1])
        # print(self.pi[2][2])
    ...
    def find_optimal_policy(self, cost_function):
        # # Iterate states
        # for x in range(self.height):
        #     for y in range(self.width):
        ...
        #                 total_vm = total_vm + self.v_m[s_primes[0][0], s_primes[0][1]]
        #
        #             avg_reward = total_reward / (1.0*self.n_sample)
        #             avg_cost = total_cost / (1.0*self.n_sample)
        #             avg_vm = total_vm / (1.0*self.n_sample)
        ...
        #             curr_val = avg_reward - lag_costs + self.gamma * avg_vm
        ...
        for x in range(self.height):
            for y in range(self.width):
                max_q = np.max(self.q[x, y, actions])
                best_action_indices = np.where(self.q[x, y, actions] == max_q, True, False)
                # best_action_indices = np.where(abs(self.q[x, y, actions] - max_q) <= max_q/100, True, False)
    ...
    def optimal_bellman_update(self, old_v, x, y, cost_function):
        ...
        for action in range(self.n_actions):
            ...
            lag_costs = self.apply_lag * current_penalty * avg_cost
            self.q[x, y, action] = avg_reward - lag_costs + gamma_values

        self.v_m[x, y] = np.max(self.q, axis=2)[x, y]
...
def load_pi(model_path, iter_msg, log_file):
    ...
    create_iteration_agent = lambda: ValueIterationLagrange(
        env=None,
        max_iter=max_iter,
        n_actions=n_actions,
