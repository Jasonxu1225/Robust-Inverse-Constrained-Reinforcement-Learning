...
class RobustDualPolicyIteration(ABC):
    ...
    def __init__(self,
        ...
        super(RobustDualPolicyIteration, self).__init__()
    ...
    def bellman_update(self, old_v, x, y, cost_function):
        ...
        for action in range(self.n_actions):
            ...
            total += self.pi[x, y, action] * (np.min(self.q2p, axis=3)[x, y, action])
        self.v[x, y] = total
    ...
    def bellman_update_safety(self, old_v, x, y, cost_function):
        ...
        for action in range(self.n_actions):
            ...
            total += self.pi_safety[x, y, action] * np.min(self.q2p_safety, axis=3)[x, y, action]
    ...
    def predict(self, obs, state, deterministic=None):
        ...
        if self.soft_tau == 0:
            ...
            if np.all(policy_prob == 0):  # avoid overflow
                action = random.choice(self.admissible_actions)
            else:
                policy_prob = policy_prob * 1 / np.sum(policy_prob)
...
def load_robust_pi(model_path, iter_msg, log_file):
    if iter_msg == 'best':
    ...
    create_iteration_agent = lambda: RobustDualPolicyIteration(
        env=None,
        max_iter=max_iter,
        n_actions=n_actions,
