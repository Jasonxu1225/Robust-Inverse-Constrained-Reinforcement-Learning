...
class DPPO(OnPolicyWithCostAlgorithm):
    ...
    def train(self) -> None:
        ...
        total_true_cost = np.sum(self.rollout_buffer.true_costs)
        ...
        logger.record("train/total_true_cost", total_true_cost)
...
#                     self.policy.reset_noise(self.batch_size)
#
#                 reward_values, cost_values, lambda_values, log_prob, entropy = self.policy.evaluate_actions(rollout_data.observations, actions)
#                 reward_values = reward_values.flatten()
#                 cost_values = cost_values.flatten()
#                 lambda_values = lambda_values.flatten()
#                 lambda_values = th.clamp(lambda_values, self.lam_min, self.lam_max)
...
#
#                 cost_for_dual = cost_values_pred - cost_values_pred.mean()
#                 dual_loss = - th.mean(th.mul(lambda_values, cost_for_dual.detach()))
#                 # dual_loss = - th.mean(th.mul(lambda_values, cost_values_pred.detach()))
#
#                 # Value loss using the TD(gae_lambda) target
#                 reward_value_loss = F.mse_loss(rollout_data.reward_returns, reward_values_pred)
...
#                         + self.ent_coef * entropy_loss
#                         + self.reward_vf_coef * reward_value_loss
#                         + self.cost_vf_coef * cost_value_loss + 0.1*dual_loss)
#
#                 # Optimization step
#                 self.policy.optimizer.zero_grad()
#                 loss.backward()
