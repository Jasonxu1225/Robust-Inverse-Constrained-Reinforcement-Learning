...
from stable_baselines3.common.buffers import (RolloutBuffer,
                                              CustomRolloutBuffer,
                                              RolloutBufferWithCost, RolloutBufferWithCostWithOp)
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            if type(cost_function) is str:
                ...
                orig_costs = costs
            true_costs = np.array([info.get('lag_cost', 0) for info in infos])
            self.num_timesteps += env.num_envs
            ...
            rollout_buffer.add(self._last_obs, self._last_original_obs, new_obs, orig_obs, actions,
                               rewards, costs, orig_costs, true_costs, self._last_dones, reward_values,
...
class OnPolicyWithCostWithOpAlgorithm_onepolicy(BaseAlgorithm):
    def __init__(
        self,
        policy: Union[str, Type[ActorCriticPolicy]],
        env: Union[GymEnv, str],
        ...
        super(OnPolicyWithCostWithOpAlgorithm_onepolicy, self).__init__(
            policy=policy,
            env=env,
            policy_base=ActorCriticPolicy,
            learning_rate=learning_rate,
    ...
    def _setup_model(self) -> None:
        ...
        self.rollout_buffer = RolloutBufferWithCostWithOp(
            self.n_steps,
            self.observation_space,
            self.action_space,
            self.device,
    ...
    def collect_rollouts(
        self, env: VecEnv, callback: BaseCallback, rollout_buffer: RolloutBufferWithCostWithOp, n_rollout_steps: int,
        cost_function: Union[str, Callable]
    ) -> bool:
        """
        Collect rollouts using the current policy and fill a `RolloutBuffer`.
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                # Convert to pytorch tensor
                obs_tensor = th.as_tensor(self._last_obs).to(self.device)
                actions, op_actions, reward_values, cost_values, log_probs, op_log_probs = self.policy.forward(obs_tensor)

            actions = actions.cpu().numpy()
            op_actions = op_actions.cpu().numpy()

            # Rescale and perform action
            clipped_actions = actions
            clipped_op_actions = op_actions

            # Clip the actions to avoid out of bound error
            if isinstance(self.action_space, gym.spaces.Box):
                clipped_actions = np.clip(actions, self.action_space.low, self.action_space.high)
                clipped_op_actions = np.clip(op_actions, self.action_space.low, self.action_space.high)
            # true step
            if np.random.random() < self.op_strength:
                true_actions = clipped_op_actions
                mask = 0
            else:
                true_actions = clipped_actions
                mask = 1

            new_obs, rewards, dones, infos = env.step(true_actions)
            orig_obs = env.get_original_obs() if isinstance(env, VecNormalize) else new_obs
            if type(cost_function) is str:
                # Need to get cost from environment.
                costs = np.array([info.get(cost_function, 0) for info in infos])
            ...
            if isinstance(self.action_space, gym.spaces.Discrete):
                # Reshape in case of discrete action
                actions = actions.reshape(-1, 1)
                op_actions = op_actions.reshape(-1, 1)

            rollout_buffer.add(self._last_obs, self._last_original_obs, new_obs, orig_obs, actions, op_actions,
                               rewards, costs, orig_costs, self._last_dones, reward_values,
                               cost_values, log_probs, op_log_probs, mask)
            self._last_obs = new_obs
            self._last_original_obs = orig_obs
            self._last_dones = dones
    ...
    def learn(
        ...
        eval_freq: int = -1,
        n_eval_episodes: int = 5,
        tb_log_name: str = "OnPolicyWithCostWithOpAlgorithm",
        eval_log_path: Optional[str] = None,
        reset_num_timesteps: bool = True,
    ) -> "OnPolicyWithCostWithOpAlgorithm":
...
class OnPolicyWithCostWithOpAlgorithm_twopolicy(BaseAlgorithm):
    def __init__(
        self,
        policy: Union[str, Type[ActorCriticPolicy]],
        env: Union[GymEnv, str],
        ...
        super(OnPolicyWithCostWithOpAlgorithm_twopolicy, self).__init__(
            policy=policy,
            env=env,
            policy_base=ActorCriticPolicy,
            learning_rate=learning_rate,
    ...
    def _setup_model(self) -> None:
        ...
        self.op_policy = self.policy_class(
            self.observation_space,
            self.action_space,
            self.lr_schedule,
            use_sde=self.use_sde,
        ...
        self.policy = self.policy.to(self.device)
        self.op_policy = self.op_policy.to(self.device)
    ...
    def collect_rollouts(
        ...
        if self.use_sde:
            self.policy.reset_noise(env.num_envs)
            self.op_policy.reset_noise(env.num_envs)
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                obs_tensor = th.as_tensor(self._last_obs).to(self.device)
                actions, reward_values, cost_values, log_probs = self.policy.forward(obs_tensor)
                op_actions, _, _, op_log_probs = self.op_policy.forward(obs_tensor)
            actions = actions.cpu().numpy()
            op_actions = op_actions.cpu().numpy()
