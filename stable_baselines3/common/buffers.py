...
from stable_baselines3.common.type_aliases import (
    ReplayBufferSamples, RolloutBufferSamples, RolloutBufferWithCostSamples, RolloutBufferWithCostWithOpSamples)
...
class RolloutBufferWithCost(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.true_costs = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
    ...
    def add(self, obs: np.ndarray, orig_obs: np.ndarray, new_obs: np.ndarray, new_orig_obs: np.ndarray,
            action: np.ndarray, reward: np.ndarray, cost: np.ndarray, orig_cost: np.ndarray, true_cost: np.ndarray,
        ...
        self.true_costs[self.pos] = np.array(true_cost).copy()
...
class RolloutBufferWithCostWithOp(BaseBuffer):
    def __init__(
        self,
        buffer_size: int,
        observation_space: spaces.Space,
        ...
        super(RolloutBufferWithCostWithOp, self).__init__(buffer_size, observation_space, action_space, device, n_envs=n_envs)
        self.reward_gamma = reward_gamma
        self.reward_gae_lambda = reward_gae_lambda
        self.cost_gamma = cost_gamma
        self.cost_gae_lambda = cost_gae_lambda
        self.observations, self.orig_observations, self.actions, self.op_actions, self.rewards, self.advantages = None, None, None, None, None, None
        self.new_observations, self.new_orig_observations = None, None
        self.reward_returns, self.cost_returns, self.dones, self.values, self.log_probs, self.op_log_probs = None, None, None, None, None, None
        self.generator_ready = False
        self.reset()
    ...
    def reset(self) -> None:
        ...
        self.new_orig_observations = np.zeros((self.buffer_size, self.n_envs) + self.obs_shape, dtype=np.float32)
        self.actions = np.zeros((self.buffer_size, self.n_envs, self.action_dim), dtype=np.float32)
        self.op_actions = np.zeros((self.buffer_size, self.n_envs, self.action_dim), dtype=np.float32)
        self.dones = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.log_probs = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.op_log_probs = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        # Rewards
        self.rewards = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.reward_returns = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.reward_values = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        ...
        self.masks = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)

        self.generator_ready = False
        super(RolloutBufferWithCostWithOp, self).reset()
    ...
    def add(self, obs: np.ndarray, orig_obs: np.ndarray, new_obs: np.ndarray, new_orig_obs: np.ndarray,
            action: np.ndarray, op_action: np.ndarray, reward: np.ndarray, cost: np.ndarray, orig_cost: np.ndarray,
            done: np.ndarray, reward_value: th.Tensor,
            cost_value: th.Tensor,
            log_prob: th.Tensor, op_log_prob: th.Tensor, mask) -> None:

        if len(log_prob.shape) == 0:
            # Reshape 0-d tensor to avoid error
            log_prob = log_prob.reshape(-1, 1)
            op_log_prob = op_log_prob.reshape(-1, 1)
        self.observations[self.pos] = np.array(obs).copy()
        self.orig_observations[self.pos] = np.array(orig_obs).copy()
        self.new_observations[self.pos] = np.array(new_obs).copy()
        self.new_orig_observations[self.pos] = np.array(new_orig_obs).copy()
        self.actions[self.pos] = np.array(action).copy()
        self.op_actions[self.pos] = np.array(op_action).copy()
        self.dones[self.pos] = np.array(done).copy()
        self.log_probs[self.pos] = log_prob.clone().cpu().numpy()
        self.op_log_probs[self.pos] = op_log_prob.clone().cpu().numpy()
        self.rewards[self.pos] = np.array(reward).copy()
        self.reward_values[self.pos] = reward_value.clone().cpu().numpy().flatten()
        self.costs[self.pos] = np.array(cost).copy()
        self.orig_costs[self.pos] = np.array(orig_cost).copy()
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[RolloutBufferSamples, None, None]:
        ...
        # Prepare the data
        if not self.generator_ready:
            for tensor in ["orig_observations", "observations", "actions", "op_actions", "log_probs", "op_log_probs", "reward_values",
                           "reward_advantages", "reward_returns", "cost_values", "cost_advantages",
                           "cost_returns", "masks"]:
                self.__dict__[tensor] = self.swap_and_flatten(self.__dict__[tensor])
            self.generator_ready = True
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> RolloutBufferWithCostWithOpSamples:
        data = (
            self.orig_observations[batch_inds],
            self.observations[batch_inds],
            self.actions[batch_inds],
            self.op_actions[batch_inds],
            self.log_probs[batch_inds].flatten(),
            self.op_log_probs[batch_inds].flatten(),
            self.reward_values[batch_inds].flatten(),
            self.reward_advantages[batch_inds].flatten(),
            self.reward_returns[batch_inds].flatten(),
            self.cost_values[batch_inds].flatten(),
            self.cost_advantages[batch_inds].flatten(),
            self.cost_returns[batch_inds].flatten(),
            self.masks[batch_inds].flatten(),
        )
        return RolloutBufferWithCostWithOpSamples(*tuple(map(self.to_torch, data)))
...
#         self.new_orig_observations = np.zeros((self.buffer_size, self.n_envs) + self.obs_shape, dtype=np.float32)
#
#         self.op_new_observations = np.zeros((self.buffer_size, self.n_envs) + self.obs_shape, dtype=np.float32)
#         self.op_new_orig_observations = np.zeros((self.buffer_size, self.n_envs) + self.obs_shape, dtype=np.float32)
#
#         self.actions = np.zeros((self.buffer_size, self.n_envs, self.action_dim), dtype=np.float32)
#         self.op_actions = np.zeros((self.buffer_size, self.n_envs, self.action_dim), dtype=np.float32)
#
...
#         # Rewards
#         self.rewards = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
#         self.op_rewards = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
#         self.reward_returns = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
#         self.reward_values = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
#         self.reward_advantages = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
#
#         # Costs
#         self.costs = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
#         self.op_costs = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
#         self.orig_costs = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
#         self.op_orig_costs = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
#         self.cost_returns = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
#         self.cost_values = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
#         self.cost_advantages = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
#
...
#         ) -> Tuple[np.ndarray, np.ndarray]:
#
#         rewards = (1-self.op_strength)*rewards + self.op_strength*op_rewards
#         last_value = last_value.clone().cpu().numpy().flatten()
#
#         last_gae_lam = 0
#         for step in reversed(range(self.buffer_size)):
...
#                                       dones: np.ndarray) -> None:
#         self.reward_returns, self.reward_advantages = self._compute_returns_and_advantage(
#                 self.rewards, self.op_rewards, self.reward_values, self.dones, self.reward_gamma, self.reward_gae_lambda,
#                 reward_last_value, dones, self.reward_advantages
#         )
#         self.cost_returns, self.cost_advantages = self._compute_returns_and_advantage(
#                 self.costs, self.op_costs, self.cost_values, self.dones, self.cost_gamma, self.cost_gae_lambda,
#                 cost_last_value, dones, self.cost_advantages
#         )
#
#     def add(self, obs: np.ndarray, orig_obs: np.ndarray, new_obs: np.ndarray, new_orig_obs: np.ndarray,
#             op_new_obs: np.ndarray, op_new_orig_obs: np.ndarray,
#             action: np.ndarray, op_action: np.ndarray, reward: np.ndarray, op_reward: np.ndarray,
#             cost: np.ndarray, orig_cost: np.ndarray, op_cost: np.ndarray, op_orig_cost: np.ndarray,
#             done: np.ndarray, reward_value: th.Tensor, cost_value: th.Tensor,
#             log_prob: th.Tensor, op_log_prob: th.Tensor) -> None:
#
#         if len(log_prob.shape) == 0:
#             # Reshape 0-d tensor to avoid error
#             log_prob = log_prob.reshape(-1, 1)
...
#         self.new_observations[self.pos] = np.array(new_obs).copy()
#         self.new_orig_observations[self.pos] = np.array(new_orig_obs).copy()
#         self.op_new_observations[self.pos] = np.array(op_new_obs).copy()
#         self.op_new_orig_observations[self.pos] = np.array(op_new_orig_obs).copy()
#         self.actions[self.pos] = np.array(action).copy()
#         self.op_actions[self.pos] = np.array(op_action).copy()
#         self.dones[self.pos] = np.array(done).copy()
#         self.log_probs[self.pos] = log_prob.clone().cpu().numpy()
#         self.op_log_probs[self.pos] = op_log_prob.clone().cpu().numpy()
#         self.rewards[self.pos] = np.array(reward).copy()
#         self.op_rewards[self.pos] = np.array(op_reward).copy()
#         self.reward_values[self.pos] = reward_value.clone().cpu().numpy().flatten()
#         self.costs[self.pos] = np.array(cost).copy()
#         self.orig_costs[self.pos] = np.array(orig_cost).copy()
#         self.op_costs[self.pos] = np.array(op_cost).copy()
#         self.op_orig_costs[self.pos] = np.array(op_orig_cost).copy()
#         self.cost_values[self.pos] = cost_value.clone().cpu().numpy().flatten()
#         self.pos += 1
#         if self.pos == self.buffer_size:
#             self.full = True
...
#         # Prepare the data
#         if not self.generator_ready:
#             for tensor in ["orig_observations", "observations", "actions", "op_actions", "log_probs", "op_log_probs",
#                            "reward_values", "reward_advantages", "reward_returns", "cost_values", "cost_advantages",
#                            "cost_returns"]:
#                 self.__dict__[tensor] = self.swap_and_flatten(self.__dict__[tensor])
#             self.generator_ready = True
#
