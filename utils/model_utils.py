...
def get_net_arch(config, log_file):
    ...
    if 'cost_vf_layers' in config['PPO'].keys():
        ...
        separate_layers = dict(pi=config['PPO']['policy_layers'],  # Policy Layers
                               vf=config['PPO']['reward_vf_layers'])  # Value Function Layers
    if 'op_policy_layers' in config['PPO'].keys():
        separate_layers.update(op_pi=config['PPO']['op_policy_layers'])
    if 'lambda_layers' in config['PPO'].keys():
        separate_layers.update(lam=config['PPO']['lambda_layers'])
...
def load_policy_iteration_config(config, env_configs, train_env, seed, log_file):
    pi_parameters = {
        ...
        "log_file": log_file,
        "n_sample": config['iteration']['roll_sample'],
    }
    ...
    if config['iteration']["method"] == "2VI-Lag" or config['iteration']["method"] =='2SoftVI-Lag':
        pi_parameters.update({
            "op_strength": config['iteration']['op_strength'],
        })
    elif config['iteration']["method"]=='DPI':
        pi_parameters.update({
            "cost_threshold": config['iteration']['cost_threshold'],
        })
    elif config['iteration']["method"]=='RDPI' or config['iteration']["method"]=='RDVI':
        if config['group'] == 'ICRL':
            pi_parameters.update({
                "op_strength": config['iteration']['op_strength'],
                "cost_threshold": config['iteration']['cost_threshold'],
                'soft_tau': config['iteration']['soft_tau']
            })
        else:
            pi_parameters.update({
                "op_strength": config['iteration']['op_strength'],
...
def load_ppo_config(config, train_env, seed, log_file):
    ...
    if config['PPO']['method'] == 'DPPO':
        ppo_parameters.update({
            "lam_lr": config['PPO']['lam_lr'],
            "lam_min": config['PPO']['lam_min'],
            "lam_max": config['PPO']['lam_max'],
        })
    if config['PPO']['method'] == 'RDPPO':
        ppo_parameters.update({
            "lam_lr": config['PPO']['lam_lr'],
            "lam_min": config['PPO']['lam_min'],
            "lam_max": config['PPO']['lam_max'],
            "op_strength": config['PPO']['op_strength'],
            "op_coef": config['PPO']['op_coef'],
            "op_type": config['PPO']['op_type'],
        })
    if config['PPO']['method'] == 'RPPO-Lag':
        ppo_parameters.update({
            "op_strength": config['PPO']['op_strength'],
            "op_coef": config['PPO']['op_coef'],
            "op_type": config['PPO']['op_type'],
