...
class MujocoExternalSignalWrapper(gym.Wrapper):
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Dict[Any, Any]]:
        ...
        if 'Safety' in self.env.spec.id:
            info.update({'env_true_cost': info['cost']})
            del info['cost']
            lag_cost_true = info['env_true_cost']
        else:
            ture_cost_function = get_true_cost_function(env_id=self.spec.id,
                                                        env_configs=self.wrapper_config)

            if 'next_state_without_wind' in info.keys():
                lag_cost_true = int(ture_cost_function(obs, action) == True
                           or ture_cost_function(info['next_state_without_wind'], action) == True)
            else:
                lag_cost_true = int(ture_cost_function(obs, action) == True)
        info.update({'lag_cost': lag_cost_true})
...
def is_mujoco(env_id):
    mujoco_env_id = ['HC', 'AntWall', 'Pendulum', 'Walker', 'LGW', 'WGW', 'Swimmer', 'Safety']
