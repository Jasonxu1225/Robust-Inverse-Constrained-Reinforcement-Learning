...
def get_true_cost_function(env_id, env_configs={}):
    ...
    if env_id in ["HCWithPos-v0",
    ...
    elif env_id in ['HCWithTwoPos-v0',
                    'HCWithTwoPosNoise-v0']:
        ...
        return partial(wall_infront_and_behind, 0.015)
    elif env_id in ["InvertedPendulumAngle-v0",
                    ]:
        return partial(wall_angle_pendulum, 0.15)
    elif env_id in ["WalkerWithPos-v0",
                    "WalkerWithPosNoise-v0",
                    "WalkerWithPosTest-v0", ]:
        return partial(wall_behind, -3)
    elif env_id in ["WalkerWithAngle-v0",
                    "WalkerWithAngleNoise-v0",]:
        return partial(wall_angle_walker, 0.6)
    elif env_id in ["WGW-v0", "WindyWGW-v0"]:
...
def wall_infront_and_behind(pos, obs, acs):
    return ((obs[..., 0] < -pos) or (obs[..., 0] > pos))
...
def wall_behind_and_angle(pos, max_angle, obs, acs):
    return ((abs(obs[..., 2]) > max_angle) or (obs[..., 0] < pos))

def wall_angle_walker(max_angle, obs, acs):
    return (abs(obs[..., 3]) > max_angle)

def wall_angle_pendulum(max_angle, obs, acs):
    # return (abs(obs[..., 2]) > max_angle)
    # return (abs(obs[..., 3]) > max_angle or abs(obs[..., 6]) > max_angle)
    # return abs(obs[..., 2]) > max_angle
    # return abs(obs[..., 2]) > max_angle
    return (abs(obs[..., 1]) > max_angle)

def wall_pos_angle_pendulum(max_pos, max_angle, obs, acs):
    return (abs(obs[..., 1]) > max_angle or abs(obs[..., 0]) > max_pos)
