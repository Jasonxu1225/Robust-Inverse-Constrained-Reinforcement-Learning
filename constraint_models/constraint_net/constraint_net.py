...
class ShiftedSigmoid(nn.Module):
    ...
    def forward(self, x):
        return th.sigmoid(x - self.shift)
...
class ConstraintNet(nn.Module):
    def __init__(
        ...
        self.regularize_all = regularize_all
    ...
    def train_traj_nn(
        ...
        total_obs = []
        total_acs = []
        for i in range(env_configs['map_height']):
            for j in range(env_configs['map_width']):
                total_obs.append(np.array([i,j]))
                total_acs.append([0])
        total_obs = np.array(total_obs)
        total_acs = np.array(total_acs)
        ...
        total_grids = self.prepare_data(total_obs, total_acs)
        ...
        for itr in tqdm(range(iterations)):
            for gid in range(min(len(nominal_data_games), len(expert_data_games))):
                ...
                for nom_batch_indices, exp_batch_indices in self.get(nominal_data.shape[0], expert_data.shape[0]):
                    ...
                    grid_preds = self.__call__(grid_data)
                    grid_preds_all.append(grid_preds)

                # Calculate loss
                expert_preds_all = th.concat(expert_preds_all)
                nominal_preds_all = th.concat(nominal_preds_all)
                grid_preds_all = th.concat(grid_preds_all)
                if self.train_gail_lambda:
                    ...
                    if self.regularize_all:
                        regularizer_loss = self.regularizer_coeff * (th.mean(1 - grid_preds_all))
                    else:
