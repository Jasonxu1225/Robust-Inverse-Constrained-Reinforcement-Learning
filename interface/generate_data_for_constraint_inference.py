...
def run():
    ...
    if config_path is not None:
        ...
        env_configs = {}
    if 'Noise' in config['env']['train_env_id']:
        env_configs = {'noise_mean': config['env']['train_noise_mean'], 'noise_std': config['env']['train_noise_std'],
                       'noise_seed': 123, 'noise_type': config['env']['noise_type']}
    ...
    elif "PI" in config['group'] or "VI" in config['group']:
        model = load_pi(model_loading_path, iter_msg=iteration_msg, log_file=log_file)
    elif "RDPI" in config['group']:
        model = load_robust_pi(model_loading_path, iter_msg=iteration_msg, log_file=None)
    else:
    ...
    if is_commonroad(env_id=config['env']['train_env_id']):
        ...
        max_benchmark_num = 100 / num_threads  # max number of expert traj is 50 for mujoco
    else:
        ...
        for i in range(num_threads):
            ...
            if save_constraint_expert:
                ...
                if is_commonroad(env_id=config['env']['train_env_id']):
                    ...
                    with open(os.path.join(save_expert_data_path,
                                           'scene-{0}_len-{1}_{2}_{3}.pkl'.format(success,
                                                                              running_steps[i],
                                                                              '-'.join(termination_reasons),
                                                                              iteration_msg
                                                                                  )
