...
from stable_baselines3.iteration import PolicyIterationLagrange, DualPolicyIteration, RobustDualPolicyIteration
from common.cns_sampler import ConstrainedRLSampler
from common.cns_evaluation import evaluate_icrl_policy
from common.cns_visualization import constraint_visualization_1d, constraint_visualization_2d, traj_visualization_2d, constraint_visualization_2d_matrix
...
from constraint_models.constraint_net.constraint_matrix import ConstraintMatrix
from stable_baselines3 import PPOLagrangian, DPPO, RDPPO_onepolicy, RDPPO_twopolicy, RPPOLagrangian
...
from stable_baselines3.iteration.robust_dual_policy_interation_traj_constraint import compute_expert_density_matrix, compute_mean_expert_length
...
def train(config):
    ...
    # init sample env
    save_sample_mother_dir = os.path.join(save_model_mother_dir, "sample/")
    if not os.path.exists(save_sample_mother_dir):
        os.mkdir(save_sample_mother_dir)
    ...
    sampling_env, env_configs = make_eval_env(env_id=config['env']['sample_env_id'],
                                              config_path=config['env']['sample_config_path'],
                                              save_dir=save_sample_mother_dir,
                                              group=config['group'],
                                              ...
                                              log_file=log_file,
                                              noise_mean=config['env']['sample_noise_mean'] if 'Noise' in config['env'][
                                                  'sample_env_id'] else None,
                                              noise_std=config['env']['sample_noise_std'] if 'Noise' in config['env'][
                                                  'sample_env_id'] else None,
                                              noise_seed=seed if 'Noise' in config['env'][
                                                  'sample_env_id'] else None,
                                              noise_type=config['env']['noise_type'] if 'Noise' in config['env'][
    ...
    eval_env, _ = make_eval_env(env_id=config['env']['eval_env_id'],
    ...
    if 'WGW' in config['env']['train_env_id']:
        save_test_mother_dir2 = os.path.join(save_model_mother_dir, "test2/")
        if not os.path.exists(save_test_mother_dir2):
            os.mkdir(save_test_mother_dir2)

        eval_env2, _ = make_eval_env(env_id=config['env']['eval_env_id2'],
                                     config_path=config['env']['eval_config_path2'],
                                     save_dir=save_test_mother_dir2,
                                     group=config['group'],
                                     use_cost=config['env']['use_cost'],
                                     normalize_obs=not config['env']['dont_normalize_obs'],
                                     cost_info_str=config['env']['cost_info_str'],
    ...
    sampler = ConstrainedRLSampler(rollouts=int(config['running']['sample_rollouts']),
                                   ...
                                   env_id=config['env']['sample_env_id'],
                                   env=sampling_env,
    ...
    if isinstance(eval_env.action_space, gym.spaces.Box):
        action_low, action_high = eval_env.action_space.low, eval_env.action_space.high
    ...
    if ('ICRL' == config['group'] or 'Binary' == config['group']) and (config['CN']['cost_net_type'] == 'network'):
        ...
        cn_parameters.update({'regularize_all': config['CN']['regularize_all'], })
        constraint_net = ConstraintNet(**cn_parameters)
    elif ('ICRL' == config['group'] or 'Binary' == config['group']) and (config['CN']['cost_net_type'] == 'matrix'):
        cn_parameters.update({'no_importance_sampling': config['CN']['no_importance_sampling'], })
        cn_parameters.update({'per_step_importance_sampling': config['CN']['per_step_importance_sampling'], })
        constraint_net = ConstraintMatrix(**cn_parameters)
    ...
    if 'WGW' in config['env']['train_env_id']:
        ...
        if config['CN']['cost_net_type']!='matrix':
    ...
    if 'PPO' in config.keys():
        ...
        if config['PPO']['method'] == 'DPPO':
            create_nominal_agent = lambda: DPPO(**ppo_parameters)
        elif config['PPO']['method'] == 'RDPPO' and config['PPO']['policy_name']=='TwoCriticsWithOpMlpPolicy':
            create_nominal_agent = lambda: RDPPO_onepolicy(**ppo_parameters)
        elif config['PPO']['method'] == 'RDPPO' and config['PPO']['policy_name']=='TwoCriticsMlpPolicy':
            create_nominal_agent = lambda: RDPPO_twopolicy(**ppo_parameters)
        elif config['PPO']['method'] == 'RPPO-Lag':
            create_nominal_agent = lambda: RPPOLagrangian(**ppo_parameters)
        else:
        ...
        if config['iteration']["method"] == 'PI-Lag':
            ...
            create_nominal_agent = lambda: DualPolicyIteration(**iteration_parameters)
        elif config['iteration']["method"] == 'RDPI':
            create_nominal_agent = lambda: RobustDualPolicyIteration(**iteration_parameters)
        elif config['iteration']["method"] == 'VI-Lag':
            create_nominal_agent = lambda: ValueIterationLagrange(**iteration_parameters)
        elif config['iteration']["method"] == '2VI-Lag':
            create_nominal_agent = lambda: TwoPlayerValueIterationLagrange(**iteration_parameters)
        elif config['iteration']["method"] == '2SoftVI-Lag':
            create_nominal_agent = lambda: TwoPlayerSoftValueIterationLagrange(**iteration_parameters)
    ...
    for itr in range(config['running']['n_iters']):
        ...
        # update lambda
        if 'PPO' in config.keys() and (config['PPO']['method'] == 'DPPO' or config['PPO']['method'] == 'RDPPO'):
            nominal_agent.lam_max += config['PPO']['lam_max_update']
        # Update agent
        ...
        if 'PPO' in config.keys() and config['PPO']['method'] == 'PPO-Lag' or \
            'PPO' in config.keys() and config['PPO']['method'] == 'DPPO' or \
            'iteration' in config.keys() and config['iteration']['method'] == 'PI-Lag' or \
            'iteration' in config.keys() and config['iteration']['method'] == 'VI-Lag':
            test_op_stength = 0
        else:
            test_op_stength = nominal_agent.op_strength
        orig_observations, observations, actions, rewards, sum_rewards, lengths = sampler.sample_from_agent(
            ...
            test_op_strength = test_op_stength
        )
        if config['running']['use_buffer']:
        ...
        train_env.set_cost_function(constraint_net.cost_function)
        sampling_env.set_cost_function(constraint_net.cost_function)
        # pass cost function to eval_env
        eval_env.set_cost_function(constraint_net.cost_function)
        ...
        # run testing
        mean_reward, std_reward, mean_nc_reward, std_nc_reward, record_infos, costs, true_costs, violation_rate = \
            evaluate_icrl_policy(nominal_agent,
                                 eval_env,
                                 render=False,
                                 ...
                                 save_path=save_path,
                                 test_op_strength=0)
        # visualize trajectory in eval env
        if 'WGW' in config['env']['train_env_id'] and itr % config['running']['save_every'] == 0:
            evaluator = ConstrainedRLSampler(rollouts=int(config['running']['sample_rollouts']),
                                           store_by_game=True,  # I move the step out
                                           cost_info_str=config['env']['cost_info_str'],
                                           sample_multi_env=sample_multi_env,
                                           ...
                                           env=eval_env,
                                           planning_config=planning_config)
            eval_orig_observations, _, _, _, _, _ = evaluator.sample_from_agent(
                policy_agent=nominal_agent,
                new_env=eval_env,
                test_op_strength=0
            )
            traj_visualization_2d(config=config,
                                  observations=eval_orig_observations,
                                  save_path=save_path,
                                  model_name=args.config_file.split('/')[-1].split('.')[0],
                                  title='Trajectory in Eval Env 1'.format(itr),
                                  )

            eval_env2.set_cost_function(constraint_net.cost_function)
            evaluator2 = ConstrainedRLSampler(rollouts=int(config['running']['sample_rollouts']),
                                           store_by_game=True,  # I move the step out
                                           cost_info_str=config['env']['cost_info_str'],
                                           sample_multi_env=sample_multi_env,
                                           env_id=config['env']['eval_env_id2'],
                                           env=eval_env2,
                                           planning_config=planning_config)
            eval_orig_observations2, _, _, _, _, _ = evaluator2.sample_from_agent(
                policy_agent=nominal_agent,
                new_env=eval_env2,
                test_op_strength=0
            )
            traj_visualization_2d(config=config,
                                  observations=eval_orig_observations2,
                                  save_path=save_path,
                                  model_name=args.config_file.split('/')[-1].split('.')[0],
                                  title='Trajectory in Eval Env 2'.format(itr),
                                  )
            ...
            if costs is not None:
                for record_info_name in config['env']["record_info_names"]:
                    ...
                    plot_curve(draw_keys=[record_info_name],
                               ...
                               ylabel='cost',
                               save_name=os.path.join(save_path, "{0}_with_predicted_cost".format(record_info_name)),
                               apply_scatter=True
                               )
                for record_info_name in config['env']["record_info_names"]:
                    plot_record_infos, plot_costs = zip(*sorted(zip(record_infos[record_info_name], true_costs)))
                    plot_curve(draw_keys=[record_info_name],
                               x_dict={record_info_name: plot_record_infos},
                               y_dict={record_info_name: plot_costs},
                               xlabel=record_info_name,
                               ylabel='cost',
                               save_name=os.path.join(save_path, "{0}_with_true_cost".format(record_info_name)),
                               apply_scatter=True
                               )
            if 'WGW' in config['env']['train_env_id']:
                # if config['CN']['cost_net_type']!='matrix':
                ...
                #                             save_path=save_path,
                #                             model_name=args.config_file.split('/')[-1].split('.')[0],
                #                             title='Recovered Constraint',
                #                             force_mode='mean',
                #                             )
                # else:
                constraint_visualization_2d_matrix(matrix=constraint_net.cost_matrix,
                                            feature_range=config['env']["visualize_info_ranges"],
                                            save_path=save_path,
                                            model_name=args.config_file.split('/')[-1].split('.')[0],
                                            title='Recovered Constraint',
                ...
                for record_info_idx in range(len(config['env']["record_info_names"])):
                    ...
                    constraint_visualization_1d(cost_function=constraint_net.cost_function,
                                                ...
                                                save_name=os.path.join(save_path,
                                                                       "{0}_visual_with_predicted_cost.png".format(record_info_name)),
                ...
                for record_info_idx in range(len(config['env']["record_info_names"])):
                    ...
                    constraint_visualization_1d(cost_function=constraint_net.cost_function,
                                                ...
                                                device=constraint_net.device,
                                                save_name=os.path.join(save_path,
                                                                       "{0}_visual_with_true_cost.png".format(record_info_name)),
                                                feature_data=plot_record_infos,
                                                feature_cost=plot_costs,
                                                feature_name=record_info_name,
                                                empirical_input_means=empirical_input_means)
        ...
        metrics = {
            ...
            "true/episode_cost_cn": np.sum(costs) / config['running']['n_eval_episodes'],
            "true/step_cost_cn": np.mean(costs),
            "true/episode_cost_true": np.sum(true_costs) / config['running']['n_eval_episodes'],
            "true/step_cost_true": np.mean(true_costs),
            "true/violation_rate_true": violation_rate,
            "best_eval/best_reward": best_true_reward
        }
